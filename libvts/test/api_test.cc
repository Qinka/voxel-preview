  ...
  for(auto x = 0; x < 2; ++x)
    for(auto y = 0; y < 2; ++y)
      for(auto z = 0; z < 2; ++z) {
        ...
        if (on.origin[i] > 0)
          for (auto j = 0; j < 24; ++j){
            EXPECT_EQ(cur[j*3+0], edgeList[j*3+0] + x - 1);
            EXPECT_EQ(cur[j*3+1], edgeList[j*3+1] + y - 1);
            EXPECT_EQ(cur[j*3+2], edgeList[j*3+2] + z - 1);
          }
        else
          for (auto j = 0; j < 72; ++j){
            EXPECT_EQ(cur[j], 0);
  ...
  for(auto x = 0; x < 2; ++x)
    for(auto y = 0; y < 2; ++y)
      for(auto z = 0; z < 2; ++z) {
        ...
        if (on.origin[i] > 0)
          for (auto j = 0; j < 24; ++j){
            EXPECT_EQ(cur[j*3+0], faceList[j*3+0] + x - 1);
            EXPECT_EQ(cur[j*3+1], faceList[j*3+1] + y - 1);
            EXPECT_EQ(cur[j*3+2], faceList[j*3+2] + z - 1);
          }
        else
          for (auto j = 0; j < 72; ++j){
            EXPECT_EQ(cur[j], 0);
